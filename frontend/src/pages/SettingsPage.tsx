...
import { UserForm, type UserFormSubmit } from '@/features/users/UserForm'
...
export default function SettingsPage() {
  ...
  // Users/Roles management is a backend-only concept — the offline
  // single-file build has no multi-user/auth concept at all (see
  // Header.tsx's single "Office Admin" identity), so these tabs simply don't
...
/**
 * Users management — fetched independently of `useAppData`'s shared
 * bootstrap, since this is admin-only security config touched only from
 * here, not business data every page reads (see the plan). All the real
 * enforcement lives on the backend (route permission middleware, the
 * USERS_ROLE_ASSIGNMENT_EDIT split); this panel just gets out of the way of
 * anyone who lacks a given action's permission.
 */
function UsersPanel() {
  ...
  const load = () => {
    Promise.all([userService.list(), roleService.list()])
      .then(([u, r]) => {
        setUsers(u)
        setRoles(r)
      })
      .catch((error) => toast.error('Could not load users', { description: error instanceof Error ? error.message : undefined }))
  }

  useEffect(load, [])

  const submit = async (values: UserFormSubmit) => {
    try {
      if (editing) {
  ...
  const toggleActive = async (user: AppUser) => {
    try {
      ...
      toast.error('Could not change status', { description: error instanceof Error ? error.message : undefined })
    }
  }
  ...
  const remove = async (user: AppUser) => {
    try {
      ...
      toast.error('Could not remove user', { description: error instanceof Error ? error.message : undefined })
    }
  }
  ...
  const resetPassword = async (password: string, passwordConfirmation: string) => {
    ...
    try {
      await userService.resetPassword(resetTarget.id, password, passwordConfirmation)
      toast.success(`Password reset for ${resetTarget.name}`)
    } catch (error) {
      toast.error('Could not reset password', { description: error instanceof Error ? error.message : undefined })
    }
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <UserForm open={formOpen} onOpenChange={setFormOpen} editing={editing} roles={roles} onSubmit={submit} />
      <ResetPasswordDialog
...
function RolesPanel() {
  ...
  const load = () => {
    roleService
      .list()
      .then(setRoles)
      .catch((error) => toast.error('Could not load roles', { description: error instanceof Error ? error.message : undefined }))
  }
  ...
  const submit = async (values: RoleInput) => {
    try {
      ...
      toast.error(editing ? 'Could not update role' : 'Could not add role', {
        description: error instanceof Error ? error.message : undefined,
      })
    }
  }
  ...
  const remove = async (role: Role) => {
    try {
      ...
      toast.error('Could not remove role', { description: error instanceof Error ? error.message : undefined })
    }
  }

  if (roles === null) return <PageSkeleton />
  ...
  return (
    <div className="space-y-4">
      ...
      <RoleForm open={formOpen} onOpenChange={setFormOpen} editing={editing} onSubmit={submit} />
    </div>
  )
}
