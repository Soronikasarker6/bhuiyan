...
export function SidebarNav({ onNavigate }: { onNavigate?: () => void }) {
  ...
  const allowed = navigation.map((item) => {
    const required = Array.isArray(item.permission) ? item.permission : [item.permission]
    return required.map((name) => usePermission(name)).some(Boolean) // eslint-disable-line react-hooks/rules-of-hooks
  })
  const visible = navigation.filter((_, index) => allowed[index])
